...
aws s3api list-objects-v2 --bucket dai-storage --query 'Contents[?StorageClass==`GLACIER`].{Key: Key}' --output text > glacier-objects.txt

while read -r key; do
    s3cmd restore s3://dai-storage/"$key"
